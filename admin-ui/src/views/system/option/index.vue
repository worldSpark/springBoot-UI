...
<!--          <el-form-item label="投票" prop="voteId">
            <el-input
...
<!--          <el-form-item label="用户昵称" prop="ticketNum">
            <el-input
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              ...
              size="mini"
              @click="handleExport"
              v-hasPermi="['system:option:export']"
            >导出</el-button>
          </el-col>
        ...
        <el-table v-loading="loading" :data="optionList" @selection-change="handleSelectionChange">
          ...
          <el-table-column v-if="thisisRegistered==1||thisisRegistered==undefined" label="操作" align="center" class-name="small-padding fixed-width">
...
<!--                <li class="list-group-item">
                ...
                <li class="list-group-item">
                  ...
                  <div class="pull-right">{{ form.phonenumber }}</div>
                </li>
                <li class="list-group-item">
                  <svg-icon icon-class="email" />用户邮箱
                  <div class="pull-right">{{ form.email }}</div>
                </li>
                <li class="list-group-item" v-if="form.dept">
                  <svg-icon icon-class="tree" />所属部门
                  <div class="pull-right" >{{ form.dept.deptName }}</div>
                </li>
                <li class="list-group-item">
                  <svg-icon icon-class="peoples" />所属角色
                  <div class="pull-right">{{ form.roleName }}</div>
                </li>
                <li class="list-group-item">
                  <svg-icon icon-class="date" />投票数
                  <div class="pull-right">{{ form.ticketNum }}</div>
                </li>-->
                <el-card class="box-card">
                  <div slot="header" class="clearfix">
...
export default {
  ...
  methods: {
    ...
    handleView(row) {
      ...
      const that = this;
      this.title = "查看用户投票详情";
      /*getUserInfoByVote(row.id).then(res => {
        that.form = res.data;
        if(!this.form.avatar){
          that.form.avatar = store.getters.avatar;
        }
        that.form.ticketNum = row.ticketNum;
      }).catch(() => {
        console.log("获取失败");
      });*/
    ...
    handleNodeClick(data) {
      ...
      this.thisisRegistered = data.isRegistered;
      this.getList();
    },
    number(index) {
