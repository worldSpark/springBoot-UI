...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		SysUser userInfo = userMapper.findUserName(username);
...
//		System.out.println("----->>userInfo=" + userInfo.getUsername() + "---"+ userInfo.getPassword());
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		List<SysRole> roles= roleMapper.findUserRole(uid);
		for(SysRole userrole:roles){
			...
			List<SysPermission> permissions=permissionMapper.findRoleId(rolid);
